...
class fid_pytorch():
    def __init__(self, opt, dataloader_val):
        ...
        if opt.gpu_ids != "-1":
            """gpu"""
            self.model_inc.cuda(int(opt.gpu_ids))
        ...
        with torch.no_grad():
            for i, data_i in enumerate(self.val_dataloader):
                ...
                if self.opt.gpu_ids != "-1":
                    image = image.cuda(int(self.opt.gpu_ids))
                image = (image + 1) / 2
    ...
    def compute_fid_with_valid_path(self, netG, netEMA):
        ...
        with torch.no_grad():
            for i, data_i in enumerate(self.val_dataloader):
                image, label,edge,img_class = models.preprocess_input(self.opt, data_i)
                if self.opt.no_EMA:
                    generated = netG(label,img_class)
                else:
                    generated = netEMA(label,img_class)
