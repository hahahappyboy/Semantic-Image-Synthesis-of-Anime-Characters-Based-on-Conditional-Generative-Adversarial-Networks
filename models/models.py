...
class OASIS_model(nn.Module):
    ...
    def forward(self, image, label, mode, losses_computer,label_canny,img_class):
        # Branching is applied to be compatible with DataParallel
        if mode == "losses_G":
            loss_G = 0
            fake = self.netG(label, img_class)
            output_D = self.netD(fake)
            loss_G_adv = losses_computer.loss(output_D, label, for_real=True,label_canny=label_canny)
            loss_G += loss_G_adv
        ...
        if mode == "losses_D":
            ...
            loss_D_fake = losses_computer.loss(output_D_fake, label, for_real=False,label_canny=label_canny)
            loss_D += loss_D_fake
            output_D_real = self.netD(image)
            loss_D_real = losses_computer.loss(output_D_real, label, for_real=True,label_canny=label_canny)
            loss_D += loss_D_real
        ...
        if mode == "generate":
            with torch.no_grad():
                if self.opt.no_EMA:
                    ...
                    fake = self.netEMA(label, img_class)
            return fake
...
def put_on_multi_gpus(model, opt):
    if opt.gpu_ids != "-1":
        ...
        """gpu"""
        model = DataParallelWithCallback(model, device_ids=gpus).cuda(int(opt.gpu_ids))
    else:
        model.module = model
...
def preprocess_input(opt, data):
    ...
    if opt.gpu_ids != "-1":
        data['label'] = data['label'].cuda(int(opt.gpu_ids))
        data['image'] = data['image'].cuda(int(opt.gpu_ids))
        data['edge'] = data['edge'].cuda()
    label_map = data['label']
    input_edges = data['edge']

    bs, _, h, w = label_map.size()
    nc = opt.semantic_nc
    if opt.gpu_ids != "-1":
        input_label = torch.FloatTensor(bs, nc, h, w).zero_().cuda(int(opt.gpu_ids))
    else:
        input_label = torch.FloatTensor(bs, nc, h, w).zero_()

    input_semantics = input_label.scatter_(1, label_map, 1.0)

    return data['image'], input_semantics,input_edges, data['class']
