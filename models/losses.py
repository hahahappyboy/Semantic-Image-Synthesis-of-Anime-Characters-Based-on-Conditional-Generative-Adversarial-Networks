...
class losses_computer():
    ...
    def loss(self, input, label,label_canny ,for_real):
        input_canny = input[:, -1:, :, :]
        input = input[:, 0:-1, :, :]
        ...
        if for_real:
            ...
            target_canny = torch.cuda.FloatTensor(1).fill_(0.0).requires_grad_(False).expand_as(label_canny).cuda(int(self.opt.gpu_ids))
...
def get_target_tensor(opt, input, target_is_real):
    if opt.gpu_ids != "-1":
        if target_is_real:
            """gpu"""
            return torch.FloatTensor(1).fill_(1.0).requires_grad_(False).expand_as(input).cuda(int(opt.gpu_ids))
        else:
            return torch.FloatTensor(1).fill_(0.0).requires_grad_(False).expand_as(input).cuda(int(opt.gpu_ids))
    else:
        if target_is_real:
